...
const GroupChatSettingsScreen = ({ route, navigation }) => {
  ...
  // ✅ פונקציה חדשה לעדכון הרשאות
  const updateGroupPermissions = async (permission, value) => {
    try {
      setUpdating(true);
      ...
      if (result.success) {
        ...
        Alert.alert('Success', 'Group permissions updated successfully');
      } else {
        Alert.alert('Error', result.message || 'Failed to update permissions');
      }
    } catch (error) {
      console.error('Update permissions error:', error);
      Alert.alert('Error', 'Problem updating permissions');
    } finally {
      setUpdating(false);
    }
  };
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.section}>
          ...
          <View style={styles.groupAvatarSection}>
            ...
            {/* ✅ תיקון - כל החברים יכולים לשנות תמונה (או לפי הגדרה) */}
        ...
        {isAdmin && (
          <View style={styles.section}>
            ...
            <View style={styles.permissionItem}>
              ...
              <Switch
                value={chatInfo?.settings?.allowNameChange !== false}
                onValueChange={(value) => updateGroupPermissions('allowNameChange', value)}
            ...
            <View style={styles.permissionItem}>
              ...
              <Switch
                value={chatInfo?.settings?.allowImageChange !== false}
                onValueChange={(value) => updateGroupPermissions('allowImageChange', value)}
                trackColor={{ false: FLAVORWORLD_COLORS.border, true: FLAVORWORLD_COLORS.primary }}
                thumbColor={FLAVORWORLD_COLORS.white}
              />
            </View>
            ...
            <View style={styles.permissionItem}>
              ...
              <Switch
                value={chatInfo?.settings?.allowMemberInvites === true}
                onValueChange={(value) => updateGroupPermissions('allowMemberInvites', value)}
                trackColor={{ false: FLAVORWORLD_COLORS.border, true: FLAVORWORLD_COLORS.primary }}
                thumbColor={FLAVORWORLD_COLORS.white}
              />
            </View>
