// components/screens/notifications/NotificationsScreen.js
...
const NotificationsScreen = ({ navigation }) => {
    ...
    switch (notification.type) {
      ...
      case 'follow':
        if (notification.fromUser?._id) {
          navigation.navigate('Profile', { userId: notification.fromUser._id });
        }
        break;
  ...
  const getNotificationMessage = (notification) => {
    const fromUserName = notification.fromUser?.fullName || notification.fromUser?.name || 'Someone';
    
    switch (notification.type) {
      case 'like':
        return `${fromUserName} liked your recipe "${notification.postTitle}"`;
      case 'comment':
        return `${fromUserName} commented on your recipe "${notification.postTitle}"`;
      case 'follow':
        return `${fromUserName} started following you`;
      case 'group_post':
        return `${fromUserName} shared a new recipe in ${notification.groupName}`;
      case 'group_join_request':
        return `${fromUserName} wants to join your group "${notification.groupName}"`;
      case 'group_request_approved':
        ...
        <View style={styles.notificationContent}>
          ...
          <View style={styles.avatarContainer}>
            <UserAvatar
              ...
              name={item.fromUser?.fullName || item.fromUser?.name || 'User'}
              size={40}
            />
            {/* אייקון סוג ההתראה */}
