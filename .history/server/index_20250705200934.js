...
app.delete('/api/auth/delete-account', async (req, res) => {
  ...
  try {
    ...
    // מצא את המשתמש (משתמש במודל שכבר מוגדר בשרת)
    const user = await User.findById(userId);
    ...
    let deletedGroupPostsCount = 0;
    let deletedChatsCount = 0;
    let deletedGroupChatsCount = 0;
    let deletedNotificationsCount = 0;

    try {
      // 1. מחיקת מתכונים רגילים
      console.log('🗑️ Deleting user recipes...');
      const recipesResult = await Recipe.deleteMany({ userId: userId });
      deletedRecipesCount = recipesResult.deletedCount || 0;
      console.log(`✅ Deleted ${deletedRecipesCount} recipes`);
    } catch (error) {
      console.warn('⚠️ Error deleting recipes:', error.message);
    }

    try {
      // 2. מחיקת פוסטי קבוצות
      console.log('🗑️ Deleting group posts...');
      const groupPostsResult = await GroupPost.deleteMany({ userId: userId });
      deletedGroupPostsCount = groupPostsResult.deletedCount || 0;
      console.log(`✅ Deleted ${deletedGroupPostsCount} group posts`);
    } catch (error) {
      console.warn('⚠️ Error deleting group posts:', error.message);
    }

    try {
      // 3. מחיקת צ'אטים פרטיים
      console.log('🗑️ Deleting private chats...');
      const chatsResult = await PrivateChat.deleteMany({
        'participants.userId': userId
      });
      deletedChatsCount = chatsResult.deletedCount || 0;
      console.log(`✅ Deleted ${deletedChatsCount} private chats`);
    } catch (error) {
      console.warn('⚠️ Error deleting private chats:', error.message);
    }

    try {
      // 4. מחיקת הודעות בצ'אטים פרטיים
      console.log('🗑️ Deleting private messages...');
      await Message.deleteMany({ senderId: userId });
      console.log('✅ Deleted private messages');
    } catch (error) {
      console.warn('⚠️ Error deleting private messages:', error.message);
    }

    try {
      // 5. הסרה מצ'אטים קבוצתיים
      console.log('🗑️ Removing from group chats...');
      ...
      console.log('✅ Removed from group chats');
    } catch (error) {
      console.warn('⚠️ Error removing from group chats:', error.message);
    }

    try {
      // 6. מחיקת הודעות בצ'אטים קבוצתיים
      console.log('🗑️ Deleting group chat messages...');
      await GroupChatMessage.deleteMany({ senderId: userId });
      console.log('✅ Deleted group chat messages');
    } catch (error) {
      console.warn('⚠️ Error deleting group chat messages:', error.message);
    }

    try {
      // 7. הסרה מקבוצות
      console.log('🗑️ Removing from groups...');
      ...
      console.warn('⚠️ Error removing from groups:', error.message);
    }
    ...
    try {
      // 9. עדכון רשימות עוקבים
      console.log('🗑️ Updating followers/following lists...');
      
      // הסר את המשתמש מרשימות העוקבים של אחרים
      await User.updateMany(
        { followers: userId },
        { $pull: { followers: userId } }
      );
      
      // הסר את המשתמש מרשימות המעקב של אחרים
      await User.updateMany(
        { following: userId },
      ...
      console.warn('⚠️ Error updating followers:', error.message);
    }

    try {
      // 10. הסרת לייקים ותגובות
      console.log('🗑️ Removing likes and comments...');
      ...
      // הסר לייקים מפוסטי קבוצות
      await GroupPost.updateMany(
        { likes: userId },
        { $pull: { likes: userId } }
      );
      
      // הסר תגובות מפוסטי קבוצות
      await GroupPost.updateMany(
        { 'comments.userId': userId },
        { $pull: { comments: { userId: userId } } }
      );
      
      console.log('✅ Removed likes and comments');
    } catch (error) {
      console.warn('⚠️ Error removing likes and comments:', error.message);
    }

    try {
      // 11. מחיקת קבוצות שהמשתמש יצר
      console.log('🗑️ Deleting user-created groups...');
      const userGroups = await Group.find({ creatorId: userId });
      
      for (const group of userGroups) {
        // מחק את כל הפוסטים של הקבוצה
        await GroupPost.deleteMany({ groupId: group._id });
        console.log(`Deleted posts for group: ${group.name}`);
      }
      
      // מחק את הקבוצות עצמן
      const deletedGroupsResult = await Group.deleteMany({ creatorId: userId });
      console.log(`✅ Deleted ${deletedGroupsResult.deletedCount} user-created groups`);
    } catch (error) {
      console.warn('⚠️ Error deleting user-created groups:', error.message);
    }

    // 12. מחיקת המשתמש עצמו
    ...
    const summary = {
      ...
      deletedGroupPosts: deletedGroupPostsCount,
