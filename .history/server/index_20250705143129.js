...
app.delete('/api/auth/delete-account', async (req, res) => {
  try {
    ...
    // Import models (וודא שהנתיבים נכונים לפרויקט שלך)
    ...
    console.log('✅ User found, proceeding with account deletion...');

    let deletedRecipesCount = 0;
    let deletedChatsCount = 0;

    // 1. מחיקת כל המתכונים של המשתמש
    ...
    // 4. הסרת לייקים מפוסטים של אחרים
    await Recipe.updateMany(
      { likes: userId },
      { $pull: { likes: userId } }
    );

    // 5. מחיקת קבצי תמונות (אם יש)
    if (user.avatar && user.avatar.includes('uploads/')) {
      ...
      try {
        if (fs.existsSync(avatarPath)) {
          ...
          console.log('🖼️ Avatar file deleted');
        }
      } catch (fileError) {
        console.error('⚠️ Failed to delete avatar file:', fileError.message);
      }
    }
...
app.delete('/api/user/delete', async (req, res) => {
  ...
  try {
    ...
    await Recipe.deleteMany({ 
      $or: [{ userId: userId }, { 'user.id': userId }, { 'user._id': userId }]
    });
    ...
    await Recipe.updateMany({}, { $pull: { likes: userId } });
...
app.put('/api/group-chats/:chatId', async (req, res) => {
  try {
    ...
    // בדוק הרשאות - כולם יכולים לשנות שם אם allowNameChange מאופשר
    if (!groupChat.settings.allowNameChange && groupChat.adminId !== currentUserId) {
      return res.status(403).json({ message: 'Only admin can modify group settings' });
    }
    ...
    console.log(`✅ Group chat updated: ${changes.join(', ')}`);
    res.json({ 
