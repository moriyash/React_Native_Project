...
const GroupSettingsScreen = ({ route, navigation }) => {
  ...
  const openImagePicker = async (source) => {
    try {
      ...
      const options = {
        mediaTypes: [ImagePicker.MediaType.Images], // שימוש במערך כמו שהתיעוד החדש דורש
        allowsEditing: true,
        aspect: [16, 9],
        quality: 0.8,
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (imageToUpload) {
          details.push('✅ Cover image saved');
        }
        if (profileUpdateData.removeImage) {
          details.push('🗑️ Cover image removed');
        }
