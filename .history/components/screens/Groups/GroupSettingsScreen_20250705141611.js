...
const GroupSettingsScreen = ({ route, navigation }) => {
  ...
  const openImagePicker = async (source) => {
    try {
      ...
      const options = {
        mediaTypes: ['images'], // API החדש - מערך של strings!
        allowsEditing: true,
        aspect: [16, 9],
        quality: 0.8,
      ...
      if (!result.canceled && result.assets?.[0]) {
        ...
        // הודעה מיידית למשתמש שהתמונה נבחרה
        Alert.alert(
          'Image Ready! 📸', 
          'Your new cover image has been selected and is ready to save. Don\'t forget to press the "Save" button to apply your changes!',
  ...
  const handleSaveChanges = async () => {
    ...
    console.log('🚀 === STARTING GROUP SAVE PROCESS ===');
    
    // הראה הודעה למשתמש שהתהליך התחיל
    if (selectedImage && selectedImage.startsWith('file://') && selectedImage !== groupData?.image) {
      Alert.alert(
        'Uploading Image 📤',
        'Your image is being uploaded to the server. This may take a moment...',
        [{ text: 'OK' }]
      );
    }
    
    console.log('🔍 Initial state check:');
    console.log('   - Original group image:', groupData?.image);
    console.log('   - Currently selected image:', selectedImage);
    console.log('   - Images are different?', selectedImage !== groupData?.image);
    console.log('   - Selected image is file URI?', selectedImage?.startsWith('file://'));
    ...
    try {
      ...
      if (canEditSettings) {
        console.log('👑 User has admin settings access');
        ...
        console.log('👤 User has basic profile access only');
      }

      // בדיקה מפורטת של תמונה
      let imageToUpload = null;
      let imageAction = 'no_change';
      ...
      if (selectedImage !== groupData?.image) {
        if (selectedImage === null) {
          ...
          imageAction = 'remove';
          console.log('🗑️ ACTION: Removing image');
        } else if (selectedImage && selectedImage.startsWith('file://')) {
          // המשתמש בחר תמונה חדשה
          imageToUpload = selectedImage;
          imageAction = 'upload_new';
          console.log('📸 ACTION: Uploading new image');
          console.log('   - New image path:', imageToUpload);
        } else if (selectedImage && selectedImage.startsWith('http')) {
          imageAction = 'keep_existing';
          console.log('🔄 ACTION: Keeping existing online image');
        }
      } else {
        console.log('⚪ ACTION: No image changes');
      }

      console.log('📦 Final update data being sent:', JSON.stringify(profileUpdateData, null, 2));
      console.log('🖼️ Image action:', imageAction);
      console.log('📁 Image to upload:', imageToUpload);

      // שליחה לשרת עם callback להתקדמות
      console.log('📡 Calling groupService.updateGroup...');
      
      // יצירת callback להתקדמות העלאה
      const onUploadProgress = (progress) => {
        console.log('📊 Upload progress received:', progress);
        setUploadProgress(progress);
      };
      
      const result = await groupService.updateGroup(groupId, profileUpdateData, imageToUpload, onUploadProgress);
      ...
      if (result.success) {
        ...
        if (result.data) {
          ...
          console.log('   - Updated image:', result.data.image);
          console.log('   - Full data keys:', Object.keys(result.data));
        }
        
        // עדכן את הסטייט המקומי
        if (result.data?.image) {
          console.log('🔄 Updating local image state to server image:', result.data.image);
          setSelectedImage(result.data.image);
        } else if (profileUpdateData.removeImage) {
          console.log('🗑️ Setting local image state to null (removed)');
        ...
        if (imageAction === 'upload_new') {
          details.push('✅ Cover image saved and uploaded to server');
        }
        if (imageAction === 'remove') {
          details.push('🗑️ Cover image removed from server');
        }
        if (formData.name !== groupData?.name) {
          details.push('📝 Group name updated');
        }
        if (formData.description !== groupData?.description) {
          details.push('📄 Description updated');
        }
        if (canEditSettings && formData.rules !== groupData?.rules) {
          details.push('📋 Rules updated');
        }
        ...
        Alert.alert('Success', successMessage, [
          { 
            text: 'OK', 
            onPress: () => {
              console.log('📤 Calling onGroupUpdated callback if available');
      ...
      setLoading(false);
      setUploadProgress(0); // אפס את התקדמות ההעלאה
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <View style={styles.imageHeader}>
            ...
            {selectedImage && selectedImage !== groupData?.image && (
              <View style={styles.changeIndicator}>
                ...
                <Text style={styles.changeText}>Ready to Save!</Text>
              </View>
            )}
          </View>
          ...
          <TouchableOpacity style={styles.imageContainer} onPress={handleImagePicker}>
            ...
            {/* באדג' שמציג שהתמונה השתנתה */}
            {selectedImage && selectedImage !== groupData?.image && selectedImage.startsWith('file://') && (
              <View style={styles.newImageBadge}>
                <Ionicons name="sparkles" size={16} color={FLAVORWORLD_COLORS.white} />
                <Text style={styles.newImageBadgeText}>NEW</Text>
              </View>
            )}
          </TouchableOpacity>
          
          {/* כפתורים לניהול תמונה */}
          <View style={styles.imageActions}>
            ...
            {/* כפתור לביטול שינויי תמונה */}
            {selectedImage && selectedImage !== groupData?.image && (
              <TouchableOpacity 
                style={styles.undoImageButton}
                onPress={() => {
                  setSelectedImage(groupData?.image || null);
                  Alert.alert('Changes Cancelled', 'Image changes have been reset to the original.');
                }}
              >
                <Ionicons name="arrow-undo-outline" size={16} color={FLAVORWORLD_COLORS.textLight} />
                <Text style={styles.undoImageText}>Undo Changes</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
