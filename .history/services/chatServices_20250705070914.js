...
async getAvailableUsersForGroupChat() {
  try {
    console.log('🔍 Getting available users for group chat...');
    console.log('Current user ID:', this.currentUser);
    
    // 1. קבל את כל הצ'אטים הפרטיים עם דיבוג מפורט
    const chatsResult = await this.getMyChats();
    
    console.log('Chats result:', chatsResult);
    console.log('Chats success:', chatsResult.success);
    console.log('Chats data length:', chatsResult.data?.length);
    console.log('First chat:', chatsResult.data?.[0]);
    ...
    chatsResult.data.forEach((chat, index) => {
      ...
      if (chat.otherUser && chat.otherUser.userId !== this.currentUser) {
        const user = {
    ...
    console.log('✅ Final available users:', uniqueUsers);
    console.log('✅ Available users count:', uniqueUsers.length);
...
// הוסף את הפונקציות האלה ל-chatServices.js

async updateGroupChat(chatId, updateData) {
  try {
    const response = await apiClient.put(`/api/group-chats/${chatId}`, updateData, {
      headers: { 'x-user-id': this.currentUser }
    });
...
async removeParticipantFromGroupChat(chatId, userId) {
  try {
    const response = await apiClient.delete(`/api/group-chats/${chatId}/participants/${userId}`, {
      headers: { 'x-user-id': this.currentUser }
    });
...
async addParticipantsToGroupChat(chatId, userIds) {
  try {
    const response = await apiClient.post(`/api/group-chats/${chatId}/participants`, {
      userIds
    }, {
      headers: { 'x-user-id': this.currentUser }
    });
...
async leaveGroupChat(chatId) {
  try {
    const response = await apiClient.delete(`/api/group-chats/${chatId}/leave`, {
      headers: { 'x-user-id': this.currentUser }
    });
