...
async getAvailableUsersForGroupChat() {
  try {
    ...
    // תיקון: הוסף /api ל-URL
    const response = await apiClient.get('/api/group-chats/available-users', {
      headers: { 'x-user-id': this.currentUser }
    });
    ...
    console.error('Error details:', error.response?.data); // הוסף את זה לדיבוג
    return { 
      success: false, 
      message: error.response?.data?.message || 'Failed to get available users' 
    };
  }
}
